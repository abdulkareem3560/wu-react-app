...
const LabelSettings = () => {
  ...
  const loadResourceBundle = async (language) => {
    try {
      const res = await fetch(`${BACKEND_BASE_URL}load-resource-bundle?lang=${language}`);
  ...
  const saveResourceBundle = async () => {
    ...
    try {
      const res = await fetch(`${BACKEND_BASE_URL}save-resource-bundle?lang=${currentLanguage}`, {
        method: 'POST',
