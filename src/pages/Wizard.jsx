...
const Wizard = () => {
  ...
  useEffect(() => {
    fetch(`${BACKEND_BASE_URL}get-layout-images`)
      .then((res) => res.json())
      .then((layoutData) => {
        ...
        if (container) {
          ...
          layoutData.forEach((item) => {
            ...
            layout.innerHTML = `
              ...
              <img src="${BACKEND_BASE_URL}${item.image}" class="uploaded-image" alt="${item.htmlFile}" />
            `;
            layout.onclick = () => {
              document
  ...
  const nextStep = () => {
    if (currentStep === 1) {
      ...
      content.innerHTML = `<iframe src="${BACKEND_BASE_URL}${selectedLayoutFile}" />`;
  ...
  const submitReceiptName = () => {
    ...
    const handleMessage = (event) => {
      ...
      `;
        ...
        fetch(`${BACKEND_BASE_URL}save-receipt-html`, {
          method: 'POST',
